...
import { ChevronRight, CircleCheckBig, Clock3, Eye, HandCoins, Hourglass, MapPin, Search, Shield, Star, Upload, UserRound } from 'lucide-react';
...
import { activities, chatThreads, contracts, disputes, escrowSummary, sidebarItems, stats } from '../data/mockData';
...
const normalizeJob = (job) => {
  ...
  const category = typeof job.category === 'string' ? job.category : job.category?.en || job.category?.vi || '';
...
function FreelancerDashboard() {
  ...
  useEffect(() => {
    const handleStorage = (event) => {
      if (event.key === 'client_jobs') {
        setSharedJobs(event.newValue ? JSON.parse(event.newValue) : []);
      }
    };
  ...
  useEffect(() => {
    if (activePage === 'marketplace') {
      const saved = localStorage.getItem('client_jobs');
      setSharedJobs(saved ? JSON.parse(saved) : []);
    }
  }, [activePage]);
  ...
  const handleApplyJob = () => {
    ...
    window.alert(`Applied to ${selectedJob.title}`);
  };
                ...
                <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
                  <div className="rounded-[24px] bg-[#091839] p-6 text-white">
                    ...
                    <div className="mt-6 grid gap-3 sm:grid-cols-2">
                      ...
                      <div className="rounded-2xl bg-white/8 p-4">
                        <p className="text-xs text-white/70">Project duration</p>
                        <p className="mt-1 text-xl font-semibold text-white">{selectedJob.delivery}</p>
                      </div>
                      <div className="rounded-2xl bg-white/8 p-4">
                        <p className="text-xs text-white/70">Freelancers needed</p>
                        <p className="mt-1 text-xl font-semibold text-white">{selectedJob.freelancersNeeded}</p>
                      </div>
                      <div className="rounded-2xl bg-white/8 p-4">
                        <p className="text-xs text-white/70">Deadline</p>
                        <p className="mt-1 text-xl font-semibold text-white">{selectedJob.deadline}</p>
                      </div>
                      <div className="rounded-2xl bg-white/8 p-4">
                        <p className="text-xs text-white/70">Hourly rate</p>
                        <p className="mt-1 text-2xl font-bold text-white">{selectedJob.hourlyRate}</p>
                      </div>
                      <div className="rounded-2xl bg-white/8 p-4">
                        <p className="text-xs text-white/70">Completed jobs</p>
                        <p className="mt-1 text-2xl font-bold text-white">{selectedJob.completedJobs}</p>
                      </div>
                      <div className="rounded-2xl bg-white/8 p-4">
                        <p className="text-xs text-white/70">Completion rate</p>
                        <p className="mt-1 text-2xl font-bold text-white">{selectedJob.completionRate}</p>
                      </div>
                      <div className="rounded-2xl bg-white/8 p-4">
                        <p className="text-xs text-white/70">Response time</p>
                        <p className="mt-1 text-2xl font-bold text-white">{selectedJob.responseTime}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3 rounded-[24px] bg-white p-4">
                    ...
                    <div className="flex flex-wrap gap-3 pt-1">
                      <button
                        ...
                        className="rounded-2xl bg-ink px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-900"
                      >
                        Apply now
                      </button>
